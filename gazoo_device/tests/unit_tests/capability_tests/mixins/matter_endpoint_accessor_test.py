...
"""Mixin tests for the MatterControllerChipTool capability."""
...
class MatterEndpointAccessorTestMixin(parameterized.TestCase):
  """Mixin tests for the MatterControllerChipTool capability implementation."""
  ...
  def test_get_endpoint_class_and_device_type_id_on_failure(self):
    ...
    self.fake_responder.behavior_dict.update(
        matter_controller_device_logs.make_device_responses((response,))
    )
    with self.assertRaisesRegex(
        errors.DeviceError, "Failed to get device type"):
      self.uut.matter_endpoints.get_endpoint_class_and_device_type_id(1)

  def test_get_supported_endpoint_ids_only_root_node(self):
    """Tests get_supported_endpoint_ids returns only root node."""
    response = {
        "cmd": (
    ...
    self.assertEqual(
        [0], self.uut.matter_endpoints.get_supported_endpoint_ids())

  def test_get_supported_clusters_empty_set(self):
    """Tests get_supported_clusters returns empty set."""
    response = {
        "cmd": (
    ...
    self.assertEqual(
        set(), self.uut.matter_endpoints.get_supported_clusters(0))
