...
class UsbHubDefaultTests(unit_test_case.UnitTestCase):
  ...
  def test_usb_hub_argument_raises_error(self):
  ...
  def test_usb_hub_set_device_power_sync(self):
  ...
  def test_usb_hub_set_device_power_charge(self):
  ...
  def test_verify_add_log_note_called(self):
  ...
  def test_add_log_note_not_called_switchboard(
      self, mock_get_switchboard_return_value):
    """Verify that add_log_note is not called if Switchboard is uninitialized or unhealthy."""
    self.get_switchboard_if_initialized.return_value = (
        mock_get_switchboard_return_value)
    uut = usb_hub_default.UsbHubDefault(
        self._device_name,
        get_manager=self.mock_get_manager,
        hub_name=self._hub_name,
        device_port=1,
        get_switchboard_if_initialized=self.get_switchboard_if_initialized)
    uut.power_off(port=1)
    self.mock_switchboard.add_log_note.assert_not_called()

  def test_verify_log_note_added_for_change_triggers_reboot(self):
  ...
  def test_verify_wait_for_bootup_complete_fn_is_called(self):
