...
class SwitchboardProcessTests(unit_test_case.MultiprocessingTestCase):
  ...
  def test_012_switchboard_process_put_message_full_queue(self):
    ...
    out_queue = self.manager.Queue(1)
  ...
  def test_020_switchboard_process_loop_pre_run_hook_returns_false(self):
    mock_start_event = MagicMock(spec=multiprocessing.synchronize.Event)
    mock_stop_event = MagicMock(spec=multiprocessing.synchronize.Event)
    mock_terminate_event = MagicMock(spec=multiprocessing.synchronize.Event)
    ...
    mock_parent_proc = MagicMock(spec=psutil.Process)

    mock_switchboard_process._pre_run_hook.return_value = False
    switchboard_process._process_loop(mock_switchboard_process,
                                      mock_parent_proc)
  ...
  def test_026_switchboard_process_loop_start_event_set_exception(self):
    """Process loop manager in parent process shutdown."""
    mock_start_event = MagicMock(spec=multiprocessing.synchronize.Event)
    mock_stop_event = MagicMock(spec=multiprocessing.synchronize.Event)
    mock_terminate_event = MagicMock(spec=multiprocessing.synchronize.Event)
  ...
  def test_030_switchboard_process_wrapper_puts_exception(self):
    ...
    with switchboard_process._child_process_wrapper(mock_parent_proc,
                                                    "fake_process",
  ...
  def test_101_switchboard_process_start_twice(self):
    ...
    self.uut = RunningProcess("fake_device", "fake_process", self.manager,
                              self.exception_queue, self.manager.Queue())
    self.uut.start()
    self.assertTrue(self.uut.is_started(),
                    "Expected process started, found not started")
  ...
  def test_103_switchboard_process_two_running_processes(self):
    ...
    uut1 = RunningProcess("fake_device", "echo_process1", self.manager,
                          self.exception_queue, self.manager.Queue())
    command_queue2 = self.manager.Queue()
    uut2 = RunningProcess("fake_device", "echo_process2", self.manager,
  ...
  def test_110_switchboard_process_exception_handler_works(self):
    ...
    self.uut = ErrorProcess("fake_device", "error_process", self.manager,
                            self.exception_queue, self.manager.Queue())
    self.uut.start()
  ...
  def test_122_switchboard_process_send_command_and_is_command_done(self):
    """Test send_command and is_command_done works."""
    command_queue = self.manager.Queue()
    self.uut = switchboard_process.SwitchboardProcess(
        "fake_device",
        "fake_command",
        ...
        command_queue,
        valid_commands=("valid_command",))
    self.assertTrue(self.uut.is_command_done(),
                    "Expected True for is_command_done")
    self.uut.send_command("valid_command", "Data")
    self.assertFalse(self.uut.is_command_done(),
                     "Expected False for is_command_done")
    command_queue.get()
    self.assertTrue(self.uut.is_command_done(),
                    "Expected True for is_command_done")
