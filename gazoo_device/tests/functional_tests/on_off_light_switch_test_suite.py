...
"""Test suite for Matter devices with on off light switch endpoint."""
...
class OnOffLightSwitchTestSuite(
    gdm_test_base.GDMTestBase,
    on_off_cluster_suite.OnOffClusterTestSuite):
  """Tests for the on off light switch endpoint."""

  def setup_class(self) -> None:
    """Sets the endpoint instance."""
    super().setup_class()
    self.endpoint = self.device.on_off_light_switch
  ...
  def is_applicable_to(cls, device_type: str,
    ...
    return gdm_test_base.whether_implements_matter_endpoint(
        device_class, device_name, "on_off_light_switch")
