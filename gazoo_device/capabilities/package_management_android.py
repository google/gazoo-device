...
The Android package management capability is used on the android devices which can have
Android APKs installed. The Android package systems rely on adb command to manage packages.
"""
...
class PackageManagementAndroid(package_management_base.PackageManagementBase):
    ...
    def install_package(self,
        ...
        """
        adb_utils.install_package_on_device(package_path,
    ...
    def uninstall_package(self, package_name):
        ...
        """
        adb_utils.uninstall_package_on_device(package_name, adb_serial=self._adb_serial)

    @decorators.CapabilityLogDecorator(logger)
    def list_packages(self):
        """List all the installed packages on the device.

        Here we use `pm list package` command to show all the packages on the device.
        Note that the results will have be "package:<package_name>".
        ...
        Raises:
            GazooDeviceError: if failed to list the packages.
        """
        _PACKAGE_PREFIX = 'package:'

        cmd_result = adb_utils.shell(self._adb_serial, COMMAND_LIST_PACKAGES)
        packages = [pkg.replace(_PACKAGE_PREFIX, '') for pkg in cmd_result.split('\n')]
        return packages
