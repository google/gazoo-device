...
"""TLV encoder and decoder module.

This module contains definitions for working with data encoded in Chip TLV format.

This is a mirror of
https://github.com/project-chip/connectedhomeip/blob/master/src/controller/python/chip/tlv/__init__.py
"""
...
import enum
import struct
from gazoo_device.utility import tlv_list
...
class float32(float):
  ...
  type offered by default in Python. This type distinction is present in the
  Matter
  data model types so we need it here as well.
  It is backed by an ordinary float, which means there will be precision loss at
  the time
...
class TLVWriter:
  ...
  def implicitProfile(self):
    ...
    profile id. Setting this value to None (the default) disabled encoding of
    implicit profile tags.
    """
  ...
  def put(self, tag, val):
    ...
    val can be a Python object which will be encoded as follows: - Python bools,
    floats and strings are encoded as their respective TLV types. - Python ints
    are encoded as unsigned TLV integers if zero or positive; signed TLV

      integers if negative.
    - None is encoded as a TLV Null.
    - bytes and bytearray objects are encoded as TVL byte strings.
    - Mapping-like objects (e.g. dict) are encoded as TLV structures.  The keys
    of the
      map object are expected to be tag values, as described below for the tag
      argument.
      Map values are encoded recursively, using the same rules as defined for
      the val
      argument. The encoding order of elements depends on the type of the map
      object.
      Elements within a dict are automatically encoded tag numerical order.
      Elements
      within other forms of mapping object (e.g. OrderedDict) are encoded in the
      object's natural iteration order.
    - Sequence-like objects (e.g. arrays) are written as TLV arrays. Elements
    within
      the array are encoded recursively, using the same rules as defined for the
      val
      argument.
    ...
    If tag is a two-integer tuple, it is encoded as a TLV profile-specific tag,
    with
      the first integer encoded as the profile id and the second as the tag
      number.
    ...
    if val is None:
      ...
      self.endContainer()
    elif isinstance(val, tlv_list.TLVList):
      self.startPath(tag)
    ...
    if isinstance(tag, tuple):
      ...
      if profile is None or profile == self._implicitProfile:
        ...
        if tagNum <= UINT16_MAX:
          ...
          return struct.pack(
              "<BHHL", controlByte, vendorId, profileNum, profile, tagNum
          )
    raise ValueError("Invalid object given for TLV tag")
...
class TLVReader:
  ...
  def _decodeControlAndTag(self, tlv, decoding):
    ...
    The control byte consists of two subfields: an element type field which
    occupies the lower 5 bits, and a tag control field which occupies the upper
    3 bits. The element type field encodes the element’s type as well as how the
    corresponding length and value fields are encoded.  In the case of Booleans
    and the null value, the element type field also encodes the value itself.
    """
  ...
  def _decodeStrLength(self, tlv, decoding):
    ...
    the element type field. If the element type needs a length field grab the
    next bytes as length
    """
  ...
  def _get(self, tlv, decodings, out):
    ...
    while len(tlv[self._bytesRead :]) and endOfEncoding == False:
      decoding = {}
      ...
      if decoding["type"] == "End of Collection":
        ...
        if "profileTag" in list(decoding.keys()):
          ...
          elif isinstance(out, tlv_list.TLVList):
