...
SPHINX_DEPRECATION_NOTICE = 'This {attr_type} has been deprecated. See "{new_name}".'


def _get_nested_attr(obj, name):
    """Get an attribute that may or may not be nested within an object.

    This will get an attribute of obj or or an attribute that is in an object that is in obj.

    Args:
        obj (object): The object to check for the attribute with the given name.
        name (str): The name of the attribute to get. name can be either an attribute of obj
                    or an object.attribute where object is part of obj.

    Returns:
        object: The method or property that may have been nested inside of other objects.
...
def add_deprecated_attribute(cls, old_name, new_name, is_method):
    ...
    added to the provided class as an alias to the "new_name". Then each time the attribute with
    "old_name" is used it will log a deprecated message and return as if the "new_name" attribute
    was called.

    Note: This must be called for a class and not for an instance of a class since dynamically
    ...
    Args:
        ...
        is_method (bool): True if the attribute is a method, or False for a property.
    """
...
def add_deprecated_attributes(cls, attribute_list):
    ...
    Args:
        cls (class): The class to add the attribute to.
        attribute_list (list): A list of tuples with the attributes to add where the tuple has:
                               old_name, new_name, is_method
    """
