...
This module contains definitions for TLV List format used by PathIB.

This is a mirror of
https://github.com/project-chip/connectedhomeip/blob/master/src/controller/python/chip/tlv/tlvlist.py
"""
...
class TLVList:
  ...
  A TLVList can be constructed from a `list` of tuples of tag and value. `None`
  stands for "anonymous tag".
  ...
  Constructs a list of three items, tag 1 is 'a', tag 2 is 'b' and with an
  anonymous item 'c'.
  ...
  def __getitem__(self, access) -> Any:
    ...
    Examples:
    ```
        tlv_list[1]                           # returns the item in the list
        with tag `1`
        tlv_list[TLVList.IndexMethod.Tag:2]   # returns the item in the list
        with tag `2`
        tlv_list[TLVList.IndexMethod.Index:0] # returns the tag and value of the
    ...
    if isinstance(access, slice):
      ...
      raise ValueError(
          "Method should be TLVList.IndexMethod.Tag or"
          " TLVList.IndexMethod.Index"
      )
  ...
  def append(self, tag: Union[None, int], value: Any) -> None:
    ...
    if (tag is not None) and (not isinstance(tag, int)):
      raise KeyError(
          f"Tag should be a integer or none for anonymous tag, {type(tag)} got"
      )
