...
"""DataFramer classes split incoming raw data into complete lines.

The NewlineFramer (default) splits incoming raw data into partial and complete
lines at newline character boundaries.

The InterwovenLogFramer finds and extracts log lines interwoven with responses
using the log line regex provided.

Notes:
A complete line returned by all DataFramer based classes should include the
trailing newline character as is.

Partial lines are typically those raw bytes that remain which don't have a
...
class DataFramer(abc.ABC):
  ...
  def get_lines(self, raw_data, begin=0, end=None):
    ...
    Yields:
        str: A partial or complete framed line found in the raw_data
        provided.
...
def _validate_end_and_begin_are_valid(begin, end, string_len):
  ...
  if not isinstance(begin, int):
    raise ValueError("Expected begin to be an integer found {}".format(
        type(begin)))
  elif begin < 0 or begin > string_len:
    raise ValueError(
        "Expected begin value to be >= 0 and <= {}, found {}".format(
            string_len, begin))
  elif end is not None:
    if not isinstance(end, int):
      raise ValueError("Expected end to be an integer found {}".format(
          type(end)))
    elif end < 0 or end > string_len:
      raise ValueError(
          "Expected end value to be >= 0 and <= {}, found {}".format(
              string_len, end))
    elif begin > end:
      raise ValueError("Expected end value {} to be <= begin value {}".format(
          end, begin))


def split_newlines_only(string,
                        begin=0,
                        end=None,
                        keepends=True,
                        cleanends=False):
  ...
  Args:
      ...
      keepends (bool): flag indicating newline character should be returned as
        part of substring returned.
...
class InterwovenLogFramer(DataFramer):
  ...
  def __init__(self,
    ...
    Args:
        log_line_pattern (str): to use to match and extract log lines from
          response lines.
        add_newline (bool): add a newline character to the extracted log
          line
    ...
    Note: The log_line_pattern should contain a single regular expression
      ...
      response line.  The cleanends flag will strip off all CR and LF
      characters at the end of the line. If keepends is True the LF
      character will be returned. This is done before the log_line_pattern
      logic is performed so that add_newline only adds a LF character to the
  ...
  def get_lines(self, raw_data, begin=0, end=None):
    ...
    Args:
        raw_data (str): to split into framed lines
        begin (int): index to the first character to start looking for
          newlines
        end (int): index to the last character to look for newline
          characters
    ...
    Note:
        ...
        past previous partial line in search of the next newline character.
    """
    for raw_line in split_newlines_only(
        raw_data,
        begin=begin,
        end=end,
...
class NewlineFramer(DataFramer):
  ...
  def __init__(self, keepends=True, cleanends=True):
    ...
    Note: The cleanends flag will strip off all CR and LF characters at the
  ...
  def get_lines(self, raw_data, begin=0, end=None):
    ...
    Returns:
        generator: A generator function that can be used to split raw_data
                   into partial or complete framed lines on newline characters.

    Note:
        The begin argument allows for a slight optimization by skipping
        past previous partial line in search of the next newline character.
    """
    return split_newlines_only(
        raw_data,
        begin=begin,
        end=end,
        keepends=self.keepends,
