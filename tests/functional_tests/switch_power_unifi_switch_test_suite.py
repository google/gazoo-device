...
"""This test suite verifies switch_power_unifi_switch capability."""

from mobly import asserts
from functional_tests import gdm_test_base


class SwitchPowerUnifiSwitchTestSuite(gdm_test_base.GDMTestBase):
    """Functional tests for the poe_switch_power capability."""
    ...
    def is_applicable_to(cls, device_type, device_class, device_name):
        ...
        return flavor == 'switch_power_unifi_switch'
    ...
    def test_3112_verify_power_on_power_off(self):
        ...
        try:
            ...
            asserts.assert_equal(self.device.switch_power.get_mode(port=1), "off",
                                 '{} port {} should have been set to {}'.format(
                                     self.device.name, '1', 'off'))
            self.device.switch_power.power_on(port=1)
            asserts.assert_equal(self.device.switch_power.get_mode(port=1), "on",
                                 '{} port {} should have been set to {}'.format(
    ...
    def test_3113_verify_set_all_port_mode(self):
        ...
        try:
            ...
            for indx, mode in enumerate(original_mode):
                if "on" in mode:
                    self.device.switch_power.power_on(port=indx + 1)
                else:
                    self.device.switch_power.power_off(port=indx + 1)
